...
let statusBarItem;
let lastHoverContent;
let globalContext;
let lastHoverTime = 0;
// 检查是否为 git 提交记录
function isGitCommit(document, position) {
    // 检查文件名是否为 git 相关
...
function updateStatusBarItem() {
    if (isEnabled) {
        ...
        const linesPerPage = config.get('linesPerPage', 2);
        ...
        statusBarItem.tooltip = new vscode.MarkdownString(`### Hover Reader\n\n` +
            `点击开启阅读器\n\n` +
            `---\n\n` +
            `$(folder) [选择文件](command:hover-reader.selectFile)\n` +
...
// 加载文本文件
async function loadTextFile(context) {
    try {
        const config = vscode.workspace.getConfiguration('hoverReader');
        let filePath = config.get('textFilePath', '');
        // 如果配置中没有路径，使用默认文件
        if (!filePath) {
...
function activate(context) {
    ...
    context.subscriptions.push(statusBarItem, toggleCommand, forwardCommand, backwardCommand, resetCommand, selectFileCommand, setLinesPerPageCommand, gotoPageCommand);
}
